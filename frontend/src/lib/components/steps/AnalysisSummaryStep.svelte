    ...
    try {
      ...
      if (mainFlow === 'baseline') {
        ...
        if (data.status === 'success') {
          ...
          console.log('📹 Risultati baseline salvati, skeleton_video_url:', data.skeleton_video_url);
        ...
        if (data.status === 'success') {
          analysisStore.setResults(data);
          console.log('📹 Risultati analisi salvati, skeleton_video_url:', data.skeleton_video_url);
          // Avanza allo step 7 (risultati) per upload method
          analysisStore.goToStep(7);
        } else {
